<template>
    <auth-layout />
    <div class="ps-60 bg-linear-to-br from-[#F8FAFC] to-[#EFF6FF] min-h-screen">
        <div class="container mx-auto p-6">
            <div class="flex flex-col h-full">
                <!-- Enhanced Header -->
                <div class="mb-8">
                    <h1 class="text-3xl font-bold text-gray-800 mb-2">Create Pre-Bidding Document</h1>
                    <p class="text-gray-600">Complete the following steps to create your pre-bidding document</p>
                </div>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                ...
                                <div class="col-span-2">
                                    ...
                                    <input 
                                        ...
                                        @input="(e) => form.step2.amount = formatNumberInput(e.target.value)"
...
<script setup>
    ...
    const closeModal = () => {
        ...
        modalForms.value.materialPlanning = { name: '', quantity: null, source: '' }
        modalForms.value.equipmentPlanning = { name: '', quantity: null }
    ...
    const submitModal = (modal) => {
        if (modal === 'biddingActivities') {
            ...
            if (selected) {
                ...
                const existingCostPlan = materialCostPlan.value.find(
                ...
                const existingMaterial = materialPlanning.value.find(
                    item => item.name === selected.name
                )
                if (existingMaterial) {
            ...
            if (selected) {
                ...
                const existingCostPlan = equipmentCostPlan.value.find(
                    item => item.name === selected.name
                )
                if (existingCostPlan) {
                    existingCostPlan.quantity += quantity
                ...
                const existingEquipment = equipmentPlanning.value.find(
                    item => item.name === selected.name
                )
    ...
    const removeMaterial = (index) => {
        ...
        if (material) {
            ...
            const costPlanIndex = materialCostPlan.value.findIndex(item => item.name === material.name)
            if (costPlanIndex !== -1) {
    ...
    const removeEquipment = (index) => {
        ...
        if (equipment) {
            // Remove from cost plan
            const costPlanIndex = equipmentCostPlan.value.findIndex(item => item.name === equipment.name)
            if (costPlanIndex !== -1) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                projectSetup: {
                    supervisor_id: form.value.step4.supervisor,
                    foreman_id: form.value.step4.foreman,
                },
                financial: {
    ...
    ===================================================== */
    const ProjectIcon = {
        template: '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>'
    }
    const BidIcon = {
    ...
    const FinancialIcon = {
        template: '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>'
    }
    ...
    ===================================================== */
    onMounted(() => {
        console.log('Create Pre-Bidding component mounted')
        fetchClients()
    })
